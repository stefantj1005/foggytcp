...
void handle_fast_retransmit(foggy_socket_t *sock, uint32_t ack) {
  ...
  // CORRECTED: Standard TCP Reno fast recovery
  ...
  // Retransmit only the oldest unacked packet (not all)
  ...
  for (auto& slot : sock->send_window) {
    ...
    if (!has_been_acked(sock, packet_seq)) {
      ...
      retransmitted = true;
      break; // Only retransmit one packet
    }
  }
  
  if (!retransmitted) {
    debug_printf("No packet to retransmit in fast recovery\n");
  }
}

/**
 * Handle additional duplicate ACKs in fast recovery
 */
void handle_dup_ack_in_fast_recovery(foggy_socket_t *sock, uint32_t ack) {
  debug_printf("Duplicate ACK in fast recovery: %d\n", ack);
  
  // In fast recovery, each duplicate ACK indicates a packet left the network
  // This is the "flight size" inflation technique
  sock->window.congestion_window += MSS;
  debug_printf("Fast recovery: CWND increased to %d\n", sock->window.congestion_window);
  
  // Try to send new data if available (if we have window space)
...
void on_recv_pkt(foggy_socket_t *sock, uint8_t *pkt) {
  ...
  if (flags == ACK_FLAG_MASK) {
    ...
    if (ack == sock->window.last_ack_received) {
      ...
      if (sock->window.reno_state == RENO_FAST_RECOVERY) {
        // In fast recovery, handle additional duplicate ACKs
        handle_dup_ack_in_fast_recovery(sock, ack);
      ...
      // Try to send more data after processing new ACK
      transmit_send_window(sock);
    } else {
      // ACK for old data - ignore
      debug_printf("Old ACK received, ignoring\n");
    }
  } 
...
void send_pkts(foggy_socket_t *sock, uint8_t *data, int buf_len) {
  ...
  // Initialize congestion control parameters if not already set
  ...
  if (sock->window.ssthresh == 0) {
    // More conservative initial ssthresh considering buffer delays
    sock->window.ssthresh = 8 * MSS;
